...
type TreeContextProps = {
  ...
  setDOMRoot: Dispatch<SetStateAction<HTMLUListElement>>;
...
const TreeContextProvider: FunctionComponent<{
  children: ReactNode;
  ...
  const [DOMRoot, setDOMRoot] = useState<HTMLUListElement>(
    {} as HTMLUListElement
  );
...
interface TreeItemProps extends HTMLAttributes<TreeItemElementInterface> {
  node: TreeNode;
  tag: TreeItemElementTag;
}
type TreeItemUtilities = {
  childrenListProps: TreeItemChildrenListProps;
  childrenListRef: RefObject<HTMLUListElement>;
  hasChildren: boolean;
  hasIcon: boolean;
  open: boolean;
};
...
>(({ node, tag, ...props }, ref: ForwardedRef<TreeItemElementInterface>) => {
  ...
  const {
    ...
    hasIcon,
    open,
  } = useMemo<TreeItemUtilities>(() => {
    const utilities: TreeItemUtilities = {
      childrenListProps: {} as TreeItemChildrenListProps,
      childrenListRef: {} as RefObject<HTMLUListElement>,
      hasChildren: !!node.children?.length,
      hasIcon: 'icon' in node,
      open: false,
    };
    if (!!node.children?.length) {
      utilities.childrenListProps = {
        id: id,
      ...
      utilities.childrenListRef = createRef<HTMLUListElement>();
      utilities.open = openTreeItemIds.includes(id);
    }
    return utilities;
      ...
      <Fragment>
        ...
        >
          ...
          >
            ...
            >
              {hasIcon && <i className="flex items-center">{node.icon}</i>}
              {value}
            </span>
          </span>
          {open && tag === 'li' && (
        ...
        {open && tag === 'a' && (
          <TreeItemChildrenList ref={childrenListRef} {...childrenListProps} />
        )}
      </Fragment>
    );
  }, [focused, inCurrentDirectory, openTreeItemIds]);
...
type TreeItemChildrenListProps = {
  id: number;
...
const TreeItemChildrenList: ForwardRefExoticComponent<TreeItemChildrenListProps &
  RefAttributes<HTMLUListElement>> = forwardRef<
  HTMLUListElement,
  TreeItemChildrenListProps
>(
  (
    { id, inCurrentDirectory, parentIsFocused, node, tag },
...
interface TreeItemListProps extends HTMLAttributes<HTMLUListElement> {
  className?: string;
  data: TreeNode;
  root?: boolean;
  tag: TreeItemElementTag;
...
const TreeItemList: ForwardRefExoticComponent<TreeItemListProps &
  RefAttributes<HTMLUListElement>> = forwardRef<
  HTMLUListElement,
  TreeItemListProps
>(
  (
    { className, data, root, tag, ...props },
    ref: ForwardedRef<HTMLUListElement>
  ): JSX.Element => {
...
const TreeContainer: ForwardRefExoticComponent<TreeProps &
  RefAttributes<HTMLUListElement>> = forwardRef<HTMLUListElement, TreeProps>(
  (
    { anchors = false, root, ...props },
    ref: ForwardedRef<HTMLUListElement>
  ): JSX.Element => {
    const {
      focusedTreeNode,
    ...
    useEffect(() => {
      setDOMRoot(
        treeContainerRef.current?.querySelector('ul') as HTMLUListElement
      );
...
export const Tree: ForwardRefExoticComponent<TreeProps &
  RefAttributes<HTMLUListElement>> = forwardRef<HTMLUListElement, TreeProps>(
  (props: TreeProps, ref: ForwardedRef<HTMLUListElement>): JSX.Element => {
    return (
      <TreeContextProvider>
        <TreeContainer ref={ref} {...props} />
      </TreeContextProvider>
    );
  }
);
